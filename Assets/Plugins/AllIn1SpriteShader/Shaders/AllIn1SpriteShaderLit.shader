...
Shader "AllIn1SpriteShader/AllIn1SpriteShaderLit"
{
   Properties
   {
      ...
      _Translucency("Translucency (Back-face Light Wrap)", Range(0, 1)) = 0 //191
   ...
   SubShader
   {
      ...
      Pass
      {
         ...
         )
         {
            ...
            // Translucency: when rendering the back face, lerp the normal toward the
            // front-face direction so light on the far side bleeds through to the player.
            // _Translucency = 0 → normal backface (dark), 1 → fully lit from the far-side light.
            if (!d.isFrontFace && _Translucency > 0)
            {
               inputData.normalWS = lerp(inputData.normalWS, -inputData.normalWS, _Translucency);
            }
